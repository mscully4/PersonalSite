...
export default function Travel(props: TravelProps) {
  ...
  const [albums, setAlbums] = useState<Record<string, TravelAlbum[]>>({});
  const [destinationCardPhotos, setDestinationCardPhotos] = useState<
    Record<string, TravelPhoto>
  >({});
  ...
  useEffect(() => {
    client.models.Destination.list().then((resp) => {
  ...
  const updateRenderablePlaces = () => {
    ...
    let closestDestination: TravelDestination;
